	...
	public function new(pack:AssetPack, name:String) {
		...
		for (element in xml.firstElement().elements()) {
			switch (element.nodeName.toLowerCase()) {
				case "texture":
					textures = [
						pack.getTexture(basePath + element.get("name").removeFileExtension() + "0"),
						pack.getTexture(basePath + element.get("name").removeFileExtension() + "1"),
						pack.getTexture(basePath + element.get("name").removeFileExtension() + "2"),
						pack.getTexture(basePath + element.get("name").removeFileExtension() + "3"),
						pack.getTexture(basePath + element.get("name").removeFileExtension() + "4"),
					];
				case "angle":
